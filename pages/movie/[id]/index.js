...
const Movie = ({ movie }) => {
  ...
  console.log(movie);
  return (
    <div className='conteiner max-w-4xl mx-auto pt-6 '>
      <div className="px-3">
        <Meta title={movie.title} />
        <Image src={`https://image.tmdb.org/t/p/original${movie.backdrop_path}`} width={1000} height={600}
...
export async function getStaticProps(context) {
  ...
  const res = await axios(`${server}/${id}?api_key=${process.env.API_KEY}&language=ru-US&page=1`)
...
export async function getStaticPaths() {
  const res = await axios(`${server}/popular?api_key=${process.env.API_KEY}&language=ru-US&page=1`);
